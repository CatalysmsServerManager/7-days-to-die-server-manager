...
module.exports = {
    ...
    fn: async function (inputs, exits) {
        ...
        for (const newCommand of newData) {
            ...
            if (!validator.isInt(newCommand.delay + '', {min: 0})) {
                problems.push(`${newCommand.delay} is not a valid delay.`)
            }

            if (!validator.isInt(newCommand.timeout + '', {min: 0})) {
                problems.push(`${newCommand.timeout} is not a valid timeout.`)
            }

            if (!validator.isInt(newCommand.costToExecute + '', {min: 0})) {
        ...
        if (problems.length === 0) {
            ...
            await CustomCommand.destroy({ server: inputs.serverId });
