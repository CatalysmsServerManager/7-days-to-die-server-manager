module.exports = async function isPlayerOwner(req, res, next) {
  ...
  try {
    ...
    if (isOwner) {
      sails.log.silly(`POLICY - isPlayerOwner - User ${user.id} is owner of the player, approving request`, {user, server, player});
      return next();
    } else {
      sails.log.warn(`POLICY - isPlayerOwner - User ${user.id} tried to access a player without being owner ${req.originalUrl}`, {user, server, player});
