module.exports = async function manageRoles(req, res, next) {
  ...
  if (_.isUndefined(req.param('serverId')) && _.isUndefined(req.query.serverId)) {
    return res.badRequest(`Error while running "manageRoles" policy, could not determine server ID.`);
  }
  ...
  if (!permCheck.hasPermission) {
    ...
    if (req.wantsJSON) {
      return res.status(403).json({
        error: `You do not have sufficient permissions! You need "manageRoles" permission. Your current role is ${permCheck.role.name}.`
      });
    } else {
      return res.view('meta/notauthorized', {
        role: permCheck.role,
        requiredPerm: 'manageRoles'
      })
    }
