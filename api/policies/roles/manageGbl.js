module.exports = async function manageGbl(req, res, next) {
    ...
    if ((_.isUndefined(req.param('serverId')) && _.isUndefined(req.query.serverId)) && _.isUndefined(req.param('playerId')) && _.isUndefined(req.query.playerId)) {
      return res.badRequest(`Error while running "manageGbl" policy, could not determine server ID or player ID.`);
    }
    ...
    if (!permCheck.hasPermission) {
      ...
      if (req.wantsJSON) {
        return res.status(403).json({
          error: `You do not have sufficient permissions! You need "manageGbl" permission. Your current role is ${permCheck.role.name}.`
        });
      } else {
        return res.view('meta/notauthorized', {
          role: permCheck.role,
          requiredPerm: 'manageGbl'
        })
      }
