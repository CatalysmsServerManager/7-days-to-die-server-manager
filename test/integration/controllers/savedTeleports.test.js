...
describe('API Saved teleports', function () {
  it('works with correct data', async function () {
    const response = await supertest(sails.hooks.http.mockApp).post(`/api/sdtdserver/teleport`)
      .send({
  ...
  it('Respects required data', async function () {
    ...
    expect(response.body).to.equal('Error while running "manageServer" policy, could not determine server ID.');
  });
  ...
  it('Deletes a teleport', async function () {
    const addResponse = await supertest(sails.hooks.http.mockApp).post(`/api/sdtdserver/teleport`)
      .send({
        serverId: sails.testServer.id,
        name: 'test',
        x: 1,
    ...
    await supertest(sails.hooks.http.mockApp).delete(`/api/sdtdserver/teleport`)
      .send({
        serverId: sails.testServer.id,
        id: teleport.id
      });
