...
describe('CustomFunction setRole', function () {
  ...
  it('Sets the players role', async () => {
    await instance.run(sails.testServer, [sails.testPlayer.id, role.name].join(','));
  ...
  it('Errors when player not found', async () => {
    return expect(instance.run(sails.testServer, [1337, role.name].join(','))).to.be.rejectedWith('Unknown player');
  });

  it('Errors when role not found', async () => {
    return expect(instance.run(sails.testServer, [sails.testPlayer.id, 1337].join(','))).to.be.rejectedWith('Unknown role');
  });
